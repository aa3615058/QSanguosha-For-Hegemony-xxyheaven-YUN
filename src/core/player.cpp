...
Player::Player(QObject *parent)
    ...
    phase(NotActive),
    weapon(NULL), armor(NULL), defensive_horse(NULL), offensive_horse(NULL), treasure(NULL), special_horse(NULL),
...
void Player::setEquip(WrappedCard *equip)
{
    ...
    switch (card->location()) {
        ...
        case EquipCard::SpecialHorseLocation: special_horse = equip; break;
    }
}
...
void Player::removeEquip(WrappedCard *equip)
{
    ...
    switch (card->location()) {
        ...
        case EquipCard::SpecialHorseLocation: special_horse = NULL; break;
    }
}
...
{
    Q_ASSERT(card != NULL);
    int weapon_id = -1, armor_id = -1, def_id = -1, off_id = -1, tr_id = -1, sp_id = -1;
    ...
    if (special_horse) sp_id = special_horse->getEffectiveId();
    QList<int> ids;
    ...
    foreach (int id, ids) {
        if (id != weapon_id && id != armor_id && id != def_id && id != off_id && id != tr_id && id != sp_id)
            return false;
    }
    return true;
}
...
WrappedCard *Player::getSpecialHorse() const
{
    return special_horse;
}
...
const EquipCard *Player::getEquip(int index) const
{
    ...
    switch (index) {
        ...
        case 5: equip = special_horse; break;
        default:
            return NULL;
    }
    if (equip != NULL)
...
int Player::getMaxCards() const
{
    ...
    origin += Sanguosha->correctMaxCards(this, false);
...
bool Player::canGetCard(const Player *to, const QString &flags) const
{
    ...
    if (flags.contains(handcard_flag) && (this != to) && (Sanguosha->isCardFixed(this, to, "h", Card::MethodGet) || to->isKongcheng()))
        hand = false;
...
bool Player::canPindianTo(const Player *to) const
{
    return this != to && !isKongcheng() && !to->isKongcheng();
}
...
bool Player::canBeChainedBy(const Player *_source) const
{
    ...
    if (isChained()) {
        ...
        if (hasArmorEffect("IronArmor") && !isBigKingdomPlayer()) {
            QList<const Player *> siblings = getAliveSiblings();
            foreach (const Player *p, siblings) {
...
QList<const Skill *> Player::getDeputySkillList(bool visible_only, bool include_acquired, bool include_equip) const
{
    ...
    QList<QString> skills;
    if (include_acquired)
        skills = deputy_skills.keys() + deputy_acquired_skills.toList();
    else
        skills = deputy_skills.keys();
    foreach (const QString &skill_name, skills) {
        const Skill *skill = Sanguosha->getSkill(skill_name);
        if (skill != NULL) {
            if ((include_equip || !hasEquipSkill(skill->objectName())) && (!visible_only || skill->isVisible()))
...
QList<const Skill *> Player::getHeadActivedSkills() const           //new function for checking skills can be actived. by weidouncle
{
    QList<const Skill *> skillList;
    QList<QString> skills = head_acquired_skills.toList();
    if (canShowGeneral("h")) skills << head_skills.keys();
    foreach (const QString &skill_name, skills) {
        const Skill *skill = Sanguosha->getSkill(skill_name);
        if (skill != NULL) {
            if (!hasEquipSkill(skill->objectName()))
                skillList << skill;
            if (skill->isVisible()) {
                QList<const Skill *> related_skill = Sanguosha->getRelatedSkills(skill->objectName());
                foreach (const Skill *s, related_skill)
...
QList<const Skill *> Player::getDeputyActivedSkills() const
{
    QList<const Skill *> skillList;
    QList<QString> skills = deputy_acquired_skills.toList();
    if (canShowGeneral("d")) skills << deputy_skills.keys();
    foreach (const QString &skill_name, skills) {
        const Skill *skill = Sanguosha->getSkill(skill_name);
        if (skill != NULL) {
            if (!hasEquipSkill(skill->objectName()))
...
QSet<QString> Player::getAcquiredSkills() const                     //this function & below get visible skills only. by weirdouncle
{
...
bool Player::isCardLimited(const Card *card, Card::HandlingMethod method, bool isHandcard) const
{
    ...
    if (method == Card::MethodUse && card->isKindOf("Peach") && getMark("GlobalBattleRoyalMode") > 0) {
        if (card->getSkillName().isEmpty() && !card->isVirtualCard())
            return true;
    }
...
void Player::copyFrom(Player *p)
{
    ...
    b->special_horse = a->special_horse;
    b->face_up = a->face_up;
    b->chained = a->chained;
...
bool Player::cheakSkillLocation(const QString &skill_name, bool is_head) const
{
    const Skill *skill = Sanguosha->getSkill(skill_name);
    if (skill == NULL)
        return false;
